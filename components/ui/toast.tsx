...
function ToastViewport({ className, ...props }: ToastPrimitive.Viewport.Props) {
	return (
		<ToastPrimitive.Viewport
			...
			className={cn(
				// z-100, above the z-50 dialogs: an error toast fired from
				// inside a dialog that stays open has to be readable.
	...
	return (
		<ToastPrimitive.Root
			...
			className={cn(
				...
				// `data-type` comes straight from base-ui's ToastRoot (the
...
function ToastIcon({ type }: { type: string | undefined }) {
	...
	if (type === 'success') {
		icon = (
			<CircleCheckIcon className="text-status-yes" aria-hidden="true" />
		);
	}
